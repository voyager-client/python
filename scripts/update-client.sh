...
CLIENT_ROOT="${SCRIPT_ROOT}/../voyager"
...
USERNAME=$(python "${SCRIPT_ROOT}/constants.py" USERNAME)
REPOSITORY=$(python "${SCRIPT_ROOT}/constants.py" REPOSITORY)
...
echo "export USERNAME=\"$(python ${SCRIPT_ROOT}/constants.py USERNAME)\"" >> $SETTING_FILE
echo "export REPOSITORY=\"$(python ${SCRIPT_ROOT}/constants.py REPOSITORY)\"" >> $SETTING_FILE

if [[ -z ${GEN_ROOT:-} ]]; then
    GEN_ROOT="${TEMP_FOLDER}/gen"
    echo ">>> Cloning gen repo"
    git clone --recursive https://github.com/voyager-client/gen.git "${GEN_ROOT}"
else
